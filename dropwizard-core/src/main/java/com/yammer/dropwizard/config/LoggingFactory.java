...
public class LoggingFactory {
	public static void bootstrap() {
		...
		root.addAppender(LogbackFactory.buildConsoleAppender(console,
				root.getLoggerContext(), Optional.<String> absent()));
	}
	...
	public void configure() {
		...
		if (console.isEnabled()) {
			root.addAppender(AsyncAppender.wrap(LogbackFactory
					.buildConsoleAppender(console, root.getLoggerContext(),
		...
		for (FileConfiguration file : config.getFileLoggers().values()) {
			if (file.isEnabled()) {
				root.addAppender(AsyncAppender.wrap(LogbackFactory
						.buildFileAppender(file, root.getLoggerContext(),
								file.getLogFormat())));
			}
		}
		...
		if (syslog.isEnabled()) {
			root.addAppender(AsyncAppender.wrap(LogbackFactory
					.buildSyslogAppender(syslog, root.getLoggerContext(), name,
							syslog.getLogFormat())));
		}
		...
		try {
			final ObjectName objectName = new ObjectName(
					"com.yammer:type=Logging");
			if (!server.isRegistered(objectName)) {
				server.registerMBean(
						new JMXConfigurator(root.getLoggerContext(), server,
	...
	private Logger configureLevels() {
		...
		for (Map.Entry<String, Level> entry : config.getLoggers().entrySet()) {
			((Logger) LoggerFactory.getLogger(entry.getKey())).setLevel(entry
					.getValue());
		}
