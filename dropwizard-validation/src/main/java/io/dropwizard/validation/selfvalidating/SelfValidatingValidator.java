...
public class SelfValidatingValidator implements ConstraintValidator<SelfValidating, Object> {
    ...
    private <T> List<ValidationCaller> findMethods(Class<T> annotated) {
        ResolvedTypeWithMembers annotatedType =
                memberResolver.resolve(typeResolver.resolve(annotated), annotationConfiguration, null);
        ...
        if (callers.isEmpty()) {
            log.warn(
                    "The class {} is annotated with @SelfValidating but contains no valid methods that are annotated "
                            + "with @SelfValidation",
                    annotated);
        }
        return callers;
    ...
    boolean isMethodCorrect(ResolvedMethod m) {
        if (m.getReturnType() != null) {
            log.error(
                    ...
                    m.getRawMember());
            return false;
        } else if (m.getArgumentCount() != 1
                || !m.getArgumentType(0).getErasedType().equals(ViolationCollector.class)) {
            log.error(
