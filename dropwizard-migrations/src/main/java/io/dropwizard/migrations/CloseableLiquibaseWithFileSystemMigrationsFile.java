...
public class CloseableLiquibaseWithFileSystemMigrationsFile extends CloseableLiquibase implements AutoCloseable {
    CloseableLiquibaseWithFileSystemMigrationsFile(ManagedDataSource dataSource, Database database, String file)
            throws LiquibaseException, SQLException {
        this(dataSource, database, file, FileSystems.getDefault());
    }

    CloseableLiquibaseWithFileSystemMigrationsFile(
            ManagedDataSource dataSource, Database database, String file, FileSystem fileSystem)
            throws LiquibaseException, SQLException {
        super(
                file,
    ...
    private static List<File> getRootPaths(FileSystem fileSystem) {
        ...
        try {
            rootPaths.add(new File(CloseableLiquibase.class
                    .getProtectionDomain()
                    .getCodeSource()
                    .getLocation()
                    .toURI()));
    ...
    public CloseableLiquibaseWithFileSystemMigrationsFile(ManagedDataSource dataSource, String file)
            throws LiquibaseException, SQLException {
        this(
                dataSource,
