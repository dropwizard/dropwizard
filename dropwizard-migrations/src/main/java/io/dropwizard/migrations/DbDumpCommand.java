...
public class DbDumpCommand<T extends Configuration> extends AbstractLiquibaseCommand<T> {
    ...
    public void configure(Subparser subparser) {
        ...
        primaryKeys
                .addArgument("--primary-keys")
                .action(Arguments.storeTrue())
                .dest("primary-keys")
                .help("Check for changed primary keys (default)");
        primaryKeys
                .addArgument("--ignore-primary-keys")
                .action(Arguments.storeFalse())
                .dest("primary-keys")
        ...
        foreignKeys
                .addArgument("--foreign-keys")
                .action(Arguments.storeTrue())
                .dest("foreign-keys")
                .help("Check for changed foreign keys (default)");
        foreignKeys
                .addArgument("--ignore-foreign-keys")
                .action(Arguments.storeFalse())
                .dest("foreign-keys")
        ...
        sequences
                .addArgument("--sequences")
                .action(Arguments.storeTrue())
                .dest("sequences")
                .help("Check for changed sequences (default)");
        sequences
                .addArgument("--ignore-sequences")
                .action(Arguments.storeFalse())
                .dest("sequences")
    ...
    private void generateChangeLog(
            ...
            final CatalogAndSchema catalogAndSchema,
            final DiffToChangeLog changeLogWriter,
        ...
        final SnapshotControl snapshotControl = new SnapshotControl(database, compareTypes.toArray(new Class[0]));
        final CompareControl compareControl = new CompareControl(
                ...
                compareTypes);
        final CatalogAndSchema[] compareControlSchemas =
        ...
        try {
            ...
            final DatabaseSnapshot comparisonSnapshot =
                    SnapshotGeneratorFactory.getInstance().createSnapshot(compareControlSchemas, null, snapshotControl);
            final DiffResult diffResult =
                    DiffGeneratorFactory.getInstance().compare(referenceSnapshot, comparisonSnapshot, compareControl);
