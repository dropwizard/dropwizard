...
class DbDumpCommandTest {
    ...
    private static final List<String> ATTRIBUTE_NAMES = Arrays.asList(
            "columns", "foreign-keys", "indexes", "primary-keys", "sequences", "tables", "unique-constraints", "views");
    private static DocumentBuilder xmlParser;

    private final DbDumpCommand<TestMigrationConfiguration> dumpCommand = new DbDumpCommand<>(
    ...
    void testDumpSchema() throws Exception {
        dumpCommand.run(
                null,
                new Namespace(ATTRIBUTE_NAMES.stream().collect(Collectors.toMap(a -> a, b -> true))),
                existedDbConf);
    ...
    void testDumpSchemaAndData() throws Exception {
        dumpCommand.run(
                null,
                new Namespace(Stream.concat(ATTRIBUTE_NAMES.stream(), Stream.of("data"))
                        .collect(Collectors.toMap(a -> a, b -> true))),
                existedDbConf);
    ...
    void testWriteToFile() throws Exception {
        ...
        assertThat(file).content(UTF_8).startsWith("<?xml version=\"1.1\" encoding=\"UTF-8\" standalone=\"no\"?>");
    }

    @Test
    void testHelpPage() {
        MigrationTestSupport.createSubparser(dumpCommand)
                .printHelp(new PrintWriter(new OutputStreamWriter(baos, UTF_8), true));

        assertThat(baos.toString(UTF_8))
                .isEqualToNormalizingNewlines("usage: db dump [-h] [--migrations MIGRATIONS-FILE] [--catalog CATALOG]\n"
    ...
    private static void assertCreateTable(Element changeSet) {
        ...
        assertThat(columns.item(0)).isInstanceOfSatisfying(Element.class, column -> assertThat(column)
                .satisfies(idColumn ->
                ...
                .satisfies(idColumnConstraints -> assertThat(idColumnConstraints.getAttribute("primaryKey"))
                        .isEqualTo("true"))
                .satisfies(idColumnConstraints -> assertThat(idColumnConstraints.getAttribute("primaryKeyName"))
                        .isEqualTo("PK_PERSONS")));

        assertThat(columns.item(1)).isInstanceOfSatisfying(Element.class, column -> assertThat(column)
                .satisfies(nameColumn ->
                ...
                .satisfies(nameColumnConstraints -> assertThat(nameColumnConstraints.getAttribute("nullable"))
                        .isEqualTo("false")));

        assertThat(columns.item(2)).isInstanceOfSatisfying(Element.class, column -> assertThat(column)
                .satisfies(emailColumn ->
    ...
    private static void assertInsertData(Element changeSet) {
        ...
        assertThat(columns.item(2)).isInstanceOfSatisfying(Element.class, column -> assertThat(column)
                .satisfies(nameColumn ->
                        assertThat(nameColumn.getAttribute("name")).isEqualTo("EMAIL"))
                .satisfies(nameColumn ->
                        assertThat(nameColumn.getAttribute("value")).isEqualTo("bill@smith.me")));
    }
