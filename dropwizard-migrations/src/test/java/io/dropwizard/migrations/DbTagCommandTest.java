...
class DbTagCommandTest {
    ...
    void testRun() throws Exception {
        ...
        // Tag them
        dbTagCommand.run(
                null, new Namespace(Collections.singletonMap("tag-name", Collections.singletonList("v1"))), conf);

        // Verify that the tag exists
        try (CloseableLiquibase liquibase =
                dbTagCommand.openLiquibase(conf.getDataSource(), new Namespace(Collections.emptyMap()))) {
    ...
    void testPrintHelp() throws Exception {
        ...
        MigrationTestSupport.createSubparser(dbTagCommand)
                .printHelp(new PrintWriter(new OutputStreamWriter(baos, UTF_8), true));
        assertThat(baos.toString(UTF_8.name()))
                .isEqualToNormalizingNewlines("usage: db tag [-h] [--migrations MIGRATIONS-FILE] [--catalog CATALOG]\n"
