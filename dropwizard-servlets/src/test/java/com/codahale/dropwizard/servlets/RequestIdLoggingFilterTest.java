...
import static com.codahale.dropwizard.servlets.RequestIdLoggingFilter.*;
...
public class RequestIdLoggingFilterTest {
    ...
    private final RequestIdLoggingFilter filter = new RequestIdLoggingFilter("ISO8859-1");
    ...
    public void hasClientRequestId() throws Exception {
        ...
        when(request.getHeader(HEADER_CLIENT_REQUEST)).thenReturn(clientId);
        filter.doFilter(request, response, chain);

        verify(response).addHeader(eq(HEADER_CLIENT_REQUEST), clientIdHeader.capture());
        verify(response).addHeader(eq(HEADER_SERVICE_REQUEST), serviceIdHeader.capture());
    ...
    public void noClientRequestId() throws Exception {
        ...
        when(request.getHeader(HEADER_CLIENT_REQUEST)).thenReturn(null);
        filter.doFilter(request, response, chain);
    ...
    public void invalidClientRequestId() throws Exception {
        ...
        when(request.getHeader(HEADER_CLIENT_REQUEST)).thenReturn("*");
        filter.doFilter(request, response, chain);
