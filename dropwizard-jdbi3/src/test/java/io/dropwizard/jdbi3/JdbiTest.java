...
class JdbiTest {
    ...
    void setUp() throws Exception {
        ...
        dbi.useTransaction(h -> {
            h.createScript(new String(
                            getClass().getResourceAsStream("/schema.sql").readAllBytes(), StandardCharsets.UTF_8))
                    .execute();
            h.createScript(new String(
                            getClass().getResourceAsStream("/data.sql").readAllBytes(), StandardCharsets.UTF_8))
                    .execute();
        });
    ...
    void fluentQueryWorks() {
        dbi.useHandle(h -> assertThat(h.createQuery("SELECT id FROM games " + "WHERE home_scored>visitor_scored "
    ...
    void canAcceptOptionalParams() {
        ...
        assertThat(metricRegistry.timer("game-dao.findHomeTeamByGameId").getCount())
                .isEqualTo(1);
    }
    ...
    void canReturnImmutableSets() {
        ...
        assertThat(metricRegistry.timer("game-dao.findAllUniqueHomeTeams").getCount())
                .isEqualTo(1);
    }

    @Test
    void canReturnOptional() {
        assertThat(dao.findIdByTeamsAndDate("NY Rangers", "Vancouver Canucks", LocalDate.of(2016, 5, 14)))
                .contains(2);
        assertThat(metricRegistry.timer("game-dao.findIdByTeamsAndDate").getCount())
                .isEqualTo(1);
    }

    @Test
    void canReturnEmptyOptional() {
        assertThat(dao.findIdByTeamsAndDate("Vancouver Canucks", "NY Rangers", LocalDate.of(2016, 5, 14)))
                .isEmpty();
        assertThat(metricRegistry.timer("game-dao.findIdByTeamsAndDate").getCount())
                .isEqualTo(1);
    }
    ...
    void worksWithDates() {
        ...
        assertThat(metricRegistry.timer("game-dao.getFirstPlayedSince").getCount())
                .isEqualTo(1);
    }
    ...
    void worksWithAbsentOptionalDates() {
        assertThat(dao.getLastPlayedDateByTeams("Vancouver Canucks", "NY Rangers"))
                .isEmpty();
        assertThat(metricRegistry.timer("game-dao.last-played-date").getCount()).isEqualTo(1);
    }
    ...
    void testQueriesWorkForLocalDateTimes() {
        dbi.useHandle(h -> assertThat(h
                        .createQuery("SELECT played_at FROM games " + "WHERE home_scored > visitor_scored "
                                + "AND played_at > :played_at")
                        .bind(
                                "played_at",
                                LocalDateTime.parse("2016-02-15T02:03:04").truncatedTo(ChronoUnit.DAYS))
                        .mapTo(LocalDateTime.class)
                        .stream()
                        .map(LocalDateTime::toLocalDate)
                        .collect(Collectors.toList()))
                .containsOnly(LocalDate.parse("2016-05-14"), LocalDate.parse("2016-03-10")));
    }
}
