...
public class OptionalMessageBodyWriter implements MessageBodyWriter<Optional<?>> {
    ...
    public void writeTo(
        ...
        final MessageBodyWriter writer = requireNonNull(mbw)
                .get()
                .getMessageBodyWriter(entityObj.getClass(), innerGenericType, annotations, mediaType);
        writer.writeTo(
                entityObj, entityObj.getClass(), innerGenericType, annotations, mediaType, httpHeaders, entityStream);
    }
}
