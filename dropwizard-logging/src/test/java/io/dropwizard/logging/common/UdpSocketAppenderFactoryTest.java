...
class UdpSocketAppenderFactoryTest {
    ...
    void testSendLogsByTcp() throws Exception {
        ...
        try (DatagramSocket datagramSocket = new DatagramSocket(UDP_PORT);
                UdpServer udpServer = new UdpServer(datagramSocket, messageCount)) {
            Future<List<String>> receivedMessages = udpServer.receive();
            DefaultLoggingFactory loggingFactory = new YamlConfigurationFactory<>(
                            DefaultLoggingFactory.class, BaseValidator.newValidator(), objectMapper, "dw-udp")
            ...
            List<String> loggedMessages = generateLogs(logger, messageCount).stream()
