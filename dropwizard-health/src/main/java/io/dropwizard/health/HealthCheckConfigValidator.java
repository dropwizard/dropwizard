...
class HealthCheckConfigValidator implements Managed {
    ...
    private void validateConfiguration(
            final List<HealthCheckConfiguration> healthCheckConfigs, final Set<String> registeredHealthCheckNames) {
        ...
        if (!notConfiguredHealthCheckNames.isEmpty()) {
            ...
            LOGGER.info(
                    "The following health check(s) were registered, but are not configured with a schedule:\n{}",
                    healthCheckList);
        }
        ...
        if (!notRegisteredHealthCheckNames.isEmpty()) {
            final String healthCheckList = notRegisteredHealthCheckNames.stream()
                    ...
                    .collect(Collectors.joining("\n"));
            LOGGER.error(
                    "The following health check(s) are configured with a schedule, but were not registered:\n{}",
                    healthCheckList);
            throw new IllegalStateException(
                    "The following configured health checks were not registered: " + notRegisteredHealthCheckNames);
        }
    }
}
